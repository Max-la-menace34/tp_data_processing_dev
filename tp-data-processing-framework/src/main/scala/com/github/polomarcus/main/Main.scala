...
object Main {
  def main(args: Array[String]) {
    ...
    // Tips: use a groupBy
    enrichedDataset.groupBy("containsWordGlobalWarming").count().show()
    //logger.info(s"We have ${count_climate} news on the climate in our dataset")
    ...
    enrichedDataset.createGlobalTempView("news")
    spark.sql("SELECT * FROM global_temp.news").show()
